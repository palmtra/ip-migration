...
def records_from_arista(device_dir: Path, device: str, platform: str) -> list[Record]:
    ...
    arp = _payload_for(show_commands, "ip arp")
    neighbors = []
    ...
    interfaces = _payload_for(show_commands, "ip interface brief")
    iface_map = {}
    ...
    routes = _payload_for(show_commands, "ip route")
    vrfs = []
    if isinstance(routes, dict):
        vrfs = [routes]
        if "vrfs" in routes and isinstance(routes["vrfs"], dict):
            vrfs = list(routes["vrfs"].values())
    for vrf in vrfs:
            ...
            if parse_token(prefix):
                records.append(
                    Record(
                        ...
                        field="prefix",
                        values=(prefix,),
                        context={"routeType": (data or {}).get("routeType") if isinstance(data, dict) else None},
                    )
                )

    acls = _payload_for(show_commands, "access-lists")
